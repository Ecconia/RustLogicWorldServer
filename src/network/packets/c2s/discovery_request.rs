...
impl DiscoveryRequest {
	...
	pub fn parse(mut iterator: CustomIterator) -> EhResult<DiscoveryRequest> {
		...
		let map_size = mp_reader::read_map(iterator).wrap(ex!("While reading discovery packet entry map count"))?;
		if map_size != 2 {
			return exception!("Discovery packet has wrong map size: ", map_size, " (should be ", 2, ")");
		}
		//Intention to connect:
		let key = mp_reader::read_string(iterator).wrap(ex!("While reading discovery packet key 'ForConnection'"))?;
		if String::from("ForConnection").ne(&key) {
			return exception!("Discovery packet has wrong first map key: ", key, " (should be ", "ForConnection", ")");
		}
		
		let intention_to_connect = mp_reader::read_bool(iterator).wrap(ex!("While reading discovery packet bool 'intention to connect'"))?;
		log_debug!("Wants to connect: ", intention_to_connect);
		
		let key = mp_reader::read_string(iterator).wrap(ex!("While reading discovery packet key 'RequestGUID'"))?;
		if String::from("RequestGUID").ne(&key) {
			return exception!("Discovery packet has wrong second map key: ", key, " (should be ", "RequestGUID", ")");
		}
		
		let request_uid = mp_reader::read_string(iterator).wrap(ex!("While reading discovery packet GUID string"))?;
